 ...
 */
 ...
 // GPIO numbers
 #define GPIO_CLOCK_21 (533)
 #define GPIO_LATCH_20 (532)
 #define GPIO_DATA_16 (528)
 
 // mapping of buttons numbers to buttons
 static const int button_mapping[12] = {BTN_WEST, BTN_EAST, BTN_SELECT,\
 BTN_START, ABS_HAT0Y, ABS_HAT0Y, ABS_HAT0X, ABS_HAT0X,\
 BTN_NORTH, BTN_SOUTH, BTN_TL, BTN_TR};
 ...
 static inline void reset_pins(void) {
	 ...
	 gpio_set_value(GPIO_CLOCK_21, 1);
	 gpio_set_value(GPIO_LATCH_20, 0);
 }
 
 static void init_pins(void) {
	 // set function of Pin GPIO_CLOCK_21 (GPIO 17) to output
	 // clear bits: 21, 22, 23
	 
	 gpio_direction_output(GPIO_CLOCK_21, 0);
	 // set bits: 21 (001 = OUTPUT)
	 gpio_direction_output(GPIO_LATCH_20, 0);
	 gpio_direction_input(GPIO_DATA_16);
 
 
	 gpiod_export(gpio_to_desc(GPIO_DATA_16), false);
	 gpiod_export(gpio_to_desc(GPIO_CLOCK_21), false);
	 gpiod_export(gpio_to_desc(GPIO_LATCH_20), false);
 ...
 static inline void uninit_pins(void) {
	 ...
	 gpio_set_value(GPIO_CLOCK_21, 0);
	 gpio_set_value(GPIO_LATCH_20, 0);
 }
 ...
 static void poll_snes(void) {
	 ...
	 // rise LATCH
	 gpio_set_value(GPIO_LATCH_20, 1);
	 ...
	 for (current_button=0; current_button < 10; current_button++) {
		 ...
		 uint8_t gpio_state = gpio_get_value(GPIO_DATA_16);
	 ...
	 printk(KERN_DEBUG "Enviando os dados\n");
	 input_sync(snes_dev);
 }
 ...
 static void fill_input_dev(void) {
	 ...
	 set_bit(BTN_SOUTH, snes_dev->keybit);
	 set_bit(BTN_NORTH, snes_dev->keybit);
	 set_bit(BTN_SELECT, snes_dev->keybit);
	 set_bit(BTN_START, snes_dev->keybit);
	 set_bit(BTN_WEST, snes_dev->keybit);
	 set_bit(BTN_EAST, snes_dev->keybit);
